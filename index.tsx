...
const AdBanner = ({ type, onClose }) => {
  if (type === 'interstitial') return (
    <div className="fixed inset-0 z-[100] bg-black/90 flex items-center justify-center p-6 animate-in fade-in duration-300">
      <div className="bg-[#1A1A2E] w-full max-w-sm rounded-2xl border border-[#7F5AF0]/30 overflow-hidden shadow-2xl">
        <div className="bg-[#7F5AF0] p-2 text-[10px] text-center font-bold uppercase tracking-widest">Sponsored</div>
        <div className="p-8 text-center space-y-4">
          <div className="text-4xl">🎁</div>
          <h3 className="text-xl font-bold">Cek Keberuntunganmu!</h3>
          <p className="text-sm text-gray-400">Dapatkan prediksi eksklusif dan angka hoki hari ini.</p>
          <button onClick={onClose} className="w-full bg-[#7F5AF0] py-3 rounded-xl font-bold hover:bg-[#6b48d1] transition-colors">Lanjutkan</button>
        </div>
      </div>
    </div>
  );
  return (
    <div className="w-full h-24 bg-[#1A1A2E] border border-white/5 rounded-2xl flex items-center justify-center my-4 opacity-50">
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen flex flex-col bg-mystic-gradient relative">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-20 bg-[#1A1A2E]/90 backdrop-blur-xl border-t border-white/5 flex items-center justify-around z-50 max-w-md mx-auto rounded-t-3xl">
        {navItems.map(item => (
          <button key={item.id} onClick={() => onNavigate(item.id)} className={`flex flex-col items-center gap-1 transition-all ${currentPage === item.id ? 'text-[#7F5AF0]' : 'text-gray-500'}`}>
            <item.icon size={22} />
...
const HomePage = () => {
  ...
  return (
    <div className="py-6 space-y-8 animate-in fade-in duration-500">
      ...
      <div onClick={() => setCurrentPage(Page.SEARCH)} className="bg-[#1A1A2E] p-4 rounded-2xl border border-white/10 flex items-center gap-3 text-gray-500 cursor-pointer hover:border-[#7F5AF0]/30 transition-all">
        <ICONS.Search size={20}/> <span>Cari arti mimpimu semalam...</span>
      </div>
      ...
      <section className="bg-gradient-to-br from-[#1A1A2E] to-[#0F0F1A] p-6 rounded-3xl border border-[#7F5AF0]/20 space-y-4 cursor-pointer" onClick={()=>setCurrentPage(Page.ZODIAC)}>
        <h3 className="text-xl font-bold font-cinzel">Zodiak Hari Ini</h3>
        <p className="text-xs text-gray-400 italic">Intip ramalan bintangmu di balik awan misteri.</p>
        <div className="flex gap-4 overflow-x-auto py-2 no-scrollbar">
          {['♈','♉',' Gemini','♋','♌'].map(z => <span key={z} className="text-3xl filter grayscale hover:grayscale-0 transition-all">{z}</span>)}
        </div>
      </section>
    </div>
  );
  ...
  return (
    <div className="py-6 space-y-6 animate-in slide-in-from-right duration-500">
      ...
      <div className="flex justify-between items-start">
        <h1 className="text-3xl font-cinzel font-bold leading-tight">{selectedDream.judul}</h1>
      ...
      <div className="bg-[#1A1A2E] p-6 rounded-2xl border-l-4 border-[#7F5AF0] italic text-gray-300 shadow-inner">"{selectedDream.ringkasan}"</div>
      <div className="grid grid-cols-1 gap-4">
        <div className="bg-green-500/5 p-5 rounded-2xl border border-green-500/20">
          <h4 className="text-green-400 text-xs font-bold uppercase mb-2 flex items-center gap-2"><ICONS.Star size={14}/> Sisi Terang</h4>
        ...
        <div className="bg-red-500/5 p-5 rounded-2xl border border-red-500/20">
          <h4 className="text-red-400 text-xs font-bold uppercase mb-2 flex items-center gap-2"><ICONS.Moon size={14}/> Peringatan</h4>
          <p className="text-sm text-gray-400 leading-relaxed">{selectedDream.tafsir_negatif}</p>
        </div>
      </div>
      <div className="bg-gradient-to-r from-[#7F5AF0] to-[#6b48d1] p-8 rounded-3xl text-center shadow-xl">
        <p className="text-[10px] uppercase font-bold text-white/50 tracking-widest mb-1">Angka Keberuntungan</p>
        <p className="text-5xl font-cinzel font-bold tracking-tighter text-white">{selectedDream.angka}</p>
      </div>
    </div>
  );
};
...
const AppContent = () => {
  const { 
    currentPage, setCurrentPage, isLoading, setIsLoading, 
    showInterstitial, setShowInterstitial, setSelectedDream 
  } = useAppContext();
      ...
      <AnimatePresence>
        {isLoading && (
          <motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[100] bg-black/80 flex flex-col items-center justify-center space-y-4">
            ...
            <p className="font-cinzel text-xl animate-pulse tracking-widest">Menyingkap Takdir...</p>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {currentPage === Page.SEARCH && (
        <div className="py-6 space-y-6 animate-in fade-in duration-300">
          <form onSubmit={doSearch} className="relative">
            <input autoFocus type="text" value={q} onChange={e=>setQ(e.target.value)} placeholder="Tulis mimpimu di sini..." className="w-full bg-[#1A1A2E] border border-white/10 p-5 rounded-2xl focus:outline-none focus:border-[#7F5AF0] transition-all text-lg"/>
            <button type="submit" className="absolute right-4 top-1/2 -translate-y-1/2 text-[#7F5AF0] p-2 bg-[#7F5AF0]/10 rounded-xl"><ICONS.Search size={24}/></button>
          </form>
          <div className="space-y-4 opacity-30 text-center py-20 grayscale">
            <ICONS.Dream size={64} className="mx-auto mb-4"/>
            <p className="text-sm font-cinzel uppercase tracking-[0.2em]">Cari tahu rahasia di balik tidurmu</p>
          </div>
        </div>
      )}
      {currentPage === Page.TRENDING && <div className="py-20 text-center text-gray-600 font-cinzel">Mimpi-mimpi viral segera hadir...</div>}
      {currentPage === Page.ZODIAC && <div className="py-20 text-center text-gray-600 font-cinzel">Bintang belum sejajar hari ini...</div>}
      {currentPage === Page.FAVORITE && <div className="py-20 text-center text-gray-600 font-cinzel">Simpan mimpimu untuk dilihat nanti.</div>}
    </Layout>
  );
};
